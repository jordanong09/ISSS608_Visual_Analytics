...
V(cgraph_aggregated)$color <- ifelse (V(cgraph_aggregated)$size > quantile_graph_aggregated[10], "yellow", "grey") #color yellow if vertices is top 10%
E(cgraph_aggregated)$color <- "grey"
V(cgraph_aggregated)$size <- V(cgraph_aggregated)$size/0.065 #Increase the size of nodes based on their centrality score, only those with high score will be visible
...
Through the vertices attributes of the top 10% of Eigenvector Centrality Score, we see that most participants who have high influence over the weekends attained an Education Level of `High School or College" "Bachelors` and the half of the participants fall within the age range of `56-60` and `21-25`. We will now analyse to see if these 2 factors affect how influential a participant is during the weekend. To combine the two plots together, we will use the `combine_plot` function which is a Wrapper around `plot_grid` that will return a plotgrid along with a combination of title, caption, and annotation label.

`Patchwork` library is used to plot both statistical plot on top of one another.
...
V(cgraph_aggregated_workday)$color <- ifelse (V(cgraph_aggregated_workday)$size > quantile_graph_aggregated_workday[10], "yellow", "grey") 
...
Through the vertices attributes of the top 10% of Eigenvector Centrality Score, we see that most participants who have high influence over the working dats **do not have kids** and most of them have high `joviality`. We will now analyse to see if these 2 factors affect how influential a participant is during working days. To combine the two plots together, we will use the `combine_plot` function which is a Wrapper around `plot_grid` that will return a plotgrid along with a combination of title, caption, and annotation label.
